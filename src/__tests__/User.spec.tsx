...
test('Create user flow', async () => {
  ...
  expect(result.current.modalOpen['modal-create']).toBe(undefined)
  expect(
    store.getState().user.users.filter(user => user.name === FAKE_CREATE_USER_DATA.name)
      .length
  ).toBeGreaterThan(0)
})
...
test('Update user flow', async () => {
  ...
  expect(
    store
      ...
      .user.users.filter(
        user =>
          user.id === FAKE_UPDATE_USER_DATA.id &&
...
test('Delete user flow', async () => {
  ...
  expect(store.getState().user.users.filter(user => user.id === selectedId).length).toBe(
    0
  )
})
